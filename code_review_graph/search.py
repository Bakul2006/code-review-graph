...
_FTS_STATE_TABLE = "nodes_fts_state"

# Metadata flag recording that the mirror describes the live index. Only
# rebuild_fts_index sets it, so a database whose index was last written
# without a mirror (every database built before this change) takes one full
# rebuild before any delta is allowed to delete from the index.
_FTS_STATE_METADATA_KEY = "fts_state_synced"
_FTS_STATE_VERSION = "1"
...
def _ensure_fts_state_table(conn: sqlite3.Connection) -> None:
    """Create the index-state mirror if this database predates it."""
    conn.execute(
        f"CREATE TABLE IF NOT EXISTS {_FTS_STATE_TABLE} ("  # nosec B608
    ...
    conn.execute(
        ...
        f"ON {_FTS_STATE_TABLE}(file_path)"  # nosec B608
    )
...
def rebuild_fts_index(store: GraphStore) -> int:
    ...
    try:
        ...
        conn.execute(
            ...
            "(node_id, name, qualified_name, file_path, signature) "
    ...
    if paths:
        ...
        ids.update(
            ...
            for row in conn.execute(
                ...
                "WHERE n.name IS NOT s.name "
                "OR n.qualified_name IS NOT s.qualified_name "
                "OR n.file_path IS NOT s.file_path "
                "OR n.signature IS NOT s.signature"
            )
        )
...
def _apply_fts_delta(conn: sqlite3.Connection, rowids: list[int]) -> tuple[int, int]:
    ...
    delete_sql = (
        ...
        "file_path, signature) VALUES('delete', ?, ?, ?, ?, ?)"
    )
    insert_sql = (
    ...
    for batch in _batched(rowids):
        ...
        old_rows = [
            tuple(row)
            for row in conn.execute(
                f"SELECT node_id, name, qualified_name, file_path, signature "  # nosec B608
        ...
        new_rows = [
            tuple(row)
            for row in conn.execute(
                f"SELECT id, name, qualified_name, file_path, signature "  # nosec B608
